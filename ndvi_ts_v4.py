...
class NDVIForecaster:
    ...
    def prepare_data(self):
        ...
        if self.test_df is None:
            print("Baseline data:", self.baseline_df.shape, self.baseline_df)
            print("Bas")
        else:
    ...
    def create_model(self, n_features):
        model = Sequential([
            LSTM(self.lstm_units, activation='relu', return_sequences=True, input_shape=(self.n_steps_in, n_features)),
    ...
    def train_models(self):
        ...
        self.model_original.fit(X_train, y_train, epochs=100, batch_size=32, validation_split=0.2)
        self.model_filtered.fit(X_smoothed, y_smoothed, epochs=100, batch_size=32, validation_split=0.2)

    def predict_future(self, model, input_sequence):
        predictions_scaled = []
        for i in range(0, len(input_sequence) - self.n_steps_in, self.n_steps_out):
            input_seq = input_sequence[i:i+self.n_steps_in].reshape(1, self.n_steps_in, -1)
            prediction = model.predict(input_seq)
            predictions_scaled.extend(prediction[0])
    ...
    def visualize_results(self, test_pred_original, test_pred_smoothed, 
        ...
        if self.case in [1, 2] and self.test_df is not None and not self.test_df.empty:
            fig.add_trace(go.Scatter(x=self.test_df['Date'], y=self.test_df['NDVI'], 
                                    mode='lines+markers', name='Test NDVI (Actual)', 
                                    line=dict(color='blue')))
            if test_pred_original is not None and len(test_pred_original) > 0:
                fig.add_trace(go.Scatter(x=self.test_df['Date'][:len(test_pred_original)], y=test_pred_original.flatten(),
                                        mode='lines', name='Test NDVI Predicted (Filtered+Interpolated)', 
                                        line=dict(color='lightblue', dash='dot')))
            if test_pred_smoothed is not None and len(test_pred_smoothed) > 0:
                fig.add_trace(go.Scatter(x=self.test_df['Date'][:len(test_pred_smoothed)], y=test_pred_smoothed.flatten(),
        ...
        if self.case in [2, 3]:
            ...
            # baseline_df = self.create_historical_baseline(all_dates)
...
def main():
    ...
    n_steps_in = int(input("Enter steps in (default=30): ") or 30)
    n_steps_out = int(input("Enter steps out (default=15): ") or 15)
